"""응답 유효성 판별 (TASK-01 후속).

프로바이더가 콘텐츠 필터·레이트 리밋 등으로 거부하면, 그 거부 메시지가
캐릭터 응답 자리에 그대로 담겨 온다. 이를 채점하면 인프라 문제가
'형편없는 캐릭터 품질'로 집계되어 지표가 오염된다.

실제로 reflection-off 실행에서 3건이 "The request was rejected because it was
considered high risk"를 받아 1점으로 집계되었고, 그 탓에 Reflection 효과가
