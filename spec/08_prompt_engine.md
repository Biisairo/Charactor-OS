...
토큰 예산에 맞게 섹션을 선택·배치하여 최적의 프롬프트를 생성한다.

> **v3 변경 (SPEC-09, 2026-08-18).** 엔진은 더 이상 모듈을 참조하지 않는다.
> ReAct 뇌가 도구로 모아온 `ThoughtBundle`과 persona만 받는 **순수 조립기**다.
>
> ```python
> assemble_system_prompt(persona, bundle: ThoughtBundle) -> str
> ```
>
> 이전에는 엔진이 `memory.to_prompt()`·`knowledge.search_relevant()` 등을 직접
> 불렀고, Stage 1도 같은 검색을 돌렸다. 검색이 두 번 실행되어 비용이 새고,
> 뇌가 본 근거와 프롬프트에 실린 근거가 서로 달랐다.
>
> 조립 규칙 두 가지가 여기서 나온다.
> - 뇌가 부르지 않은 도구의 섹션은 프롬프트에 **없다**. 엔진이 대신 채우지 않는다.
> - 뇌의 응답 전략이 `[내적 사고]` 섹션으로 들어간다. 전략이 비어 있으면
>   (상한 도달) 그 섹션 자체를 만들지 않는다.
...
│ 11. Inner Thought    (선택, ~120 tokens)│  ← 뇌의 응답 전략 (SPEC-09)
│ 12. Response Guide   (필수, ~100 tokens)│  ← 응답 가이드라인
└─────────────────────────────────────────┘
